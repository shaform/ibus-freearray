		...
		for ( end = begin; end < nPhoneSeq; end++ ) {
			...
			if (pho_id != -1
					&& CheckChoose( 
						pho_id, begin, end + 1, 
						&p_phrase, selectStr, 
						selectInterval, nSelect ) && p_phrase != NULL)
				AddInterval( ptd, begin, end, pho_id, p_phrase);
#else
		...
		int nSelect, TreeDataType *ptd )
{
...
#ifdef FAFT_CHEWING
		/* Avoid some terrible effect when new length of bytes are different from old */

		int i_len = inter.to - inter.from;
		char *i_cursor = ueStrSeek(out_buf, inter.from);
		char *i_cursor_end_old = ueStrSeek(i_cursor, i_len);

		int i_byte_len = ueStrNBytes((inter.p_phr)->phrase, i_len);
		int i_byte_len_old = ueStrNBytes(i_cursor, i_len);

		int i_byte_diff = i_byte_len - i_byte_len_old;

		char *i_cursor_end = i_cursor_end_old + i_byte_diff;

		if (i_byte_diff != 0)
			/* This works even if they overlap */
			memmove(i_cursor_end, i_cursor_end_old,
					ueStrNBytes(i_cursor_end_old, ueStrLen(i_cursor_end_old))+1); /* 1 for the end */
		ueStrNCpy(
				i_cursor,
				( inter.p_phr )->phrase,
				i_len, -1);
	...
	for ( i = 0; i < nSelect; i++ ) {
		...
		int i_byte_len = ueStrNBytes(selectStr[i], i_len);
		int i_byte_len_old = ueStrNBytes(i_cursor, i_len);
